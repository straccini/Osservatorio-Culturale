...
function doGet(e) {
  ...
  // v4.28.47 — Endpoint admin one-shot (?op=nextSteps&k=sinopia2026)
  // Esegue operazioni di manutenzione via URL senza bisogno del pannello admin.
  // Chiave fissa nel codice (stessa logica di ?rinnova=): impedisce esecuzioni anonime.
  // v4.28.48 — Endpoint admin (?op=...&k=sinopia2026)
  // Operazioni di manutenzione via URL senza bisogno del pannello admin.
  if (params.k === 'sinopia2026') {
    var _ns = { ts: new Date().toISOString(), op: params.op, risultato: {} };
    if (params.op === 'nextSteps') {
      try { _ns.risultato.dedupDry = bcvDeduplicaArchivio({ dryRun: true }); } catch(e) { _ns.risultato.dedupDry = { errore: e.message }; }
      try { _ns.risultato.archiviaLegacyDry = rfArchiviaFogliLegacy({ dryRun: true }); } catch(e) { _ns.risultato.archiviaLegacyDry = { errore: e.message }; }
      try { _ns.risultato.fontiSalute = frSaluteFonti(); } catch(e) { _ns.risultato.fontiSalute = { errore: e.message }; }
    } else if (params.op === 'dedupApply') {
      try { _ns.risultato = bcvDeduplicaArchivio({ dryRun: false, cap: 400 }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'archiviaLegacy') {
      try { _ns.risultato = rfArchiviaFogliLegacy({}); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'disattivaFontiMorte') {
      // v4.28.48 — Disattiva fonti con dominio morto o endpoint 404
      try { _ns.risultato = _disattivaFontiMorte_(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'fontiBandiAudit') {
      try { _ns.risultato = _analisiCompletaFontiBandi_(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'rfConfronto') {
      try { _ns.risultato = rfConfronto(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'rfStato') {
      try { _ns.risultato = rfStato(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'rfAttiva') {
      try { _ns.risultato = rfAttiva(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'puliziaOrfaneDry') {
      try { _ns.risultato = _puliziaFontiOrfane_({ dryRun: true }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'puliziaOrfaneApply') {
      try { _ns.risultato = _puliziaFontiOrfane_({ dryRun: false }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'agentiDaFontiBandi') {
      try {
        if (typeof attivaAgentiDaFontiBandi === 'function') attivaAgentiDaFontiBandi();
        _ns.risultato = { ok: true, flag: 'USE_AGENTI_DA_FONTIBANDIV5', valore: true };
      } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'qualitaBandi') {
      // v4.28.51 — Report qualità dati bandi per fonte (solo lettura)
      try { _ns.risultato = _qualitaBandiReport_(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'aggiungiFontiSettoriali') {
      // v4.28.56 — Fonti design/museografia/museologia/allestimenti nel RegistroFonti
      var _fontiSett = [
        { id: 'artribune_design', categoria: 'news', nome: 'Artribune — Design', urlFeed: 'https://www.artribune.com/category/design/feed/', tematica: 'design,allestimenti,museografia', tier: 'A', note: 'Design italiano, allestimenti museali, architettura espositiva' },
        { id: 'domusweb', categoria: 'news', nome: 'Domus — Architettura e Design', urlFeed: 'https://www.domusweb.it/it.feed.rss', tematica: 'design,architettura,museografia', tier: 'A', note: 'Rivista storica architettura/design italiana. Progetti museali e allestimenti.' },
        { id: 'icom_italia', categoria: 'news', nome: 'ICOM Italia', urlFeed: 'https://www.icom-italia.org/feed/', tematica: 'museologia,museografia,standard', tier: 'A', note: 'Consiglio Internazionale Musei — sezione italiana. Standard museali, definizioni, politiche.' },
        { id: 'museumnext', categoria: 'news', nome: 'MuseumNext — Innovation', urlFeed: 'https://www.museumnext.com/feed/', tematica: 'museologia,innovazione,allestimenti', tier: 'B', note: 'Innovazione museale internazionale. Allestimenti, tecnologie espositive, audience development.' },
        { id: 'aam_us', categoria: 'news', nome: 'AAM — American Alliance of Museums', urlFeed: 'https://www.aam-us.org/feed/', tematica: 'museologia,gestione,best_practice', tier: 'B', note: 'Standard e best practice museali internazionali. Progettazione, accessibilita, gestione.' }
      ];
      var _sh = _rfSheet_(true);
      if (!_sh) { _ns.risultato = { errore: 'Foglio RegistroFonti assente' }; }
      else {
        var _idx = _rfIndiceUrl_(_sh);
        var _aggiunte = [], _duplicate = [];
        _fontiSett.forEach(function(f) {
          if (_idx[_rfNorm_(f.urlFeed)]) { _duplicate.push(f.nome); return; }
          var riga = _rfRiga_(RF_HEADERS, f);
          _sh.appendRow(riga);
          _aggiunte.push(f.nome);
          _idx[_rfNorm_(f.urlFeed)] = true;
        });
        _ns.risultato = { ok: true, aggiunte: _aggiunte, duplicate: _duplicate };
      }
    } else if (params.op === 'aggiungiFontiBandiNuove') {
      // v4.28.55 — Aggiunge AgenziaCult e PNRR Cultura come fonti RSS in FontiBandi_v5
      var _fontiNuove = [
        { id: 'agenziacult_bandi', nome: 'AgenziaCult — Bandi cultura', url: 'https://www.agenziacult.it/categoria/bandi/feed/',
          tipo: 'RSS', tag: 'settoriale', categoria: 'Aggregatore', priorita: 1, livello: 'Nazionale',
          enteDefault: '', note: 'Aggregatore quotidiano bandi cultura: MiC, regioni, fondazioni, EU. WordPress RSS.' },
        { id: 'pnrr_cultura_mic', nome: 'PNRR Cultura — MiC', url: 'https://pnrr.cultura.gov.it/feed/',
          tipo: 'RSS', tag: 'settoriale', categoria: 'Ministero', priorita: 1, livello: 'Nazionale',
          enteDefault: 'Ministero della Cultura', note: 'Feed ufficiale PNRR Cultura Missione 1 Componente 3. WordPress RSS.' }
      ];
      var _risultati = [];
      _fontiNuove.forEach(function(f) {
        try {
          var r = aggiungiFonteApiAgente({
            id: f.id, nome: f.nome, url: f.url, categoria: f.categoria,
            priorita: f.priorita, livello: f.livello, enteDefault: f.enteDefault, note: f.note
          });
          _risultati.push(r);
        } catch(e) { _risultati.push({ ok: false, nome: f.nome, errore: e.message }); }
      });
      _ns.risultato = { ok: true, fontiAggiunte: _risultati };
    } else if (params.op === 'puliziaNormeDry') {
      try { _ns.risultato = _puliziaNormeFalse_({ dryRun: true }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'puliziaNormeApply') {
      try { _ns.risultato = _puliziaNormeFalse_({ dryRun: false }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'archiviaNonBandiDry') {
      try { _ns.risultato = _archiviaBandiNonBandi_({ dryRun: true }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'archiviaNonBandiApply') {
      try { _ns.risultato = _archiviaBandiNonBandi_({ dryRun: false }); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else if (params.op === 'auditSezioni') {
      // v4.28.52 — Audit coerenza contenuti sezioni (Podcast, Pubblicazioni, Norme, LavoroCultura)
      try { _ns.risultato = _auditCoerenzaSezioni_(); } catch(e) { _ns.risultato = { errore: e.message }; }
    } else {
      _ns.risultato = { errore: 'op sconosciuta. Valide: nextSteps, dedupApply, archiviaLegacy, disattivaFontiMorte, fontiBandiAudit, rfConfronto, rfStato, rfAttiva, qualitaBandi, auditSezioni' };
    }
    return ContentService.createTextOutput(JSON.stringify(_ns, null, 2)).setMimeType(ContentService.MimeType.JSON);
  }
