...
function getWeeklyNewCounts() {
  ...
  try {
    ...
    if (sh && sh.getLastRow() > 1) {
      ...
      // v4.28.35 — Dedup DIFENSIVO, allineato a getNewsListV42 (v4.18.15):
      // il foglio Items può contenere righe duplicate perché lo scanner non
      // garantisce unicità, e il badge deve dire lo stesso numero della lista
      // che l'utente poi apre.
      // ONESTÀ SUL PERCHÉ: era stato introdotto ipotizzando che il badge
      // fosse gonfiato dai duplicati. MISURATO il 10/08 su 7 giorni mobili:
      // 1037 righe grezze, 1037 uniche, ZERO duplicati — l'ipotesi era
      // sbagliata, il volume è reale. Resta come protezione (costo
      // trascurabile, garantisce la parità badge/lista se lo scanner in
      // futuro dovesse duplicare), NON come correzione di un difetto.
      // La misura è verificabile in ?diag=contatori → newsDedup.
      var iLink  = h.indexOf('FonteURL'); if (iLink < 0) iLink = h.indexOf('Link');
      var iFonte = h.indexOf('Fonte');
      ...
      for (var i = 1; i < rows.length; i++) {
        ...
        var d = rows[i][dI];
        if (!d || (d instanceof Date ? d : new Date(d)) < lunedi) continue;
        var urlVal   = iLink  >= 0 ? String(rows[i][iLink]  || '').trim().toLowerCase().replace(/\/+$/, '') : '';
        var fonteVal = iFonte >= 0 ? String(rows[i][iFonte] || '').trim().toLowerCase() : '';
        var titVal   = iTit   >= 0 ? String(rows[i][iTit]   || '').trim().toLowerCase() : '';
        var key = urlVal || (titVal + '|' + fonteVal);
        if (key && seenNews[key]) continue;
        if (key) seenNews[key] = true;
        counts.news++;
      }
    }
  } catch(e) {}
...
function diagContatoriBadge() {
  ...
  // v4.28.35 — QUANTO PESA IL DEDUP NEWS. Il badge "nuovi" contava le righe
  // grezze del foglio Items, che contiene duplicati (lo scanner non garantisce
  // unicita'). Qui la misura diretta su finestra MOBILE di 7 giorni — non "dal
  // lunedi", altrimenti il lunedi mattina il campione e' di poche ore e non
  // dice nulla. Sola lettura: serve a verificare il fix con un numero, non
  // con un'ipotesi.
  try {
    var ssn = getMainSS();
    var shn = ssn.getSheetByName(SH.ITEMS);
    var nd = { finestra: '7 giorni mobili', grezze: 0, uniche: 0, duplicate: 0 };
    if (shn && shn.getLastRow() > 1) {
      var nv = shn.getDataRange().getValues(), nh = nv[0];
      var ndI = nh.indexOf('DataAcquisizione');
      var naI = nh.indexOf('Archiviato');
      var nlI = nh.indexOf('FonteURL'); if (nlI < 0) nlI = nh.indexOf('Link');
      var nfI = nh.indexOf('Fonte');
      var ntI = nh.indexOf('Titolo');
      var cutoff = Date.now() - 7 * 86400000;
      var seenD = {};
      for (var z = 1; z < nv.length; z++) {
        if (!nv[z][0]) continue;
        if (naI >= 0 && (nv[z][naI] === true || String(nv[z][naI]).toUpperCase() === 'TRUE')) continue;
        var zd = _wkParseData_(nv[z][ndI]);
        if (!zd || zd.getTime() < cutoff) continue;
        nd.grezze++;
        var zu = nlI >= 0 ? String(nv[z][nlI] || '').trim().toLowerCase().replace(/\/+$/, '') : '';
        var zf = nfI >= 0 ? String(nv[z][nfI] || '').trim().toLowerCase() : '';
        var zt = ntI >= 0 ? String(nv[z][ntI] || '').trim().toLowerCase() : '';
        var zk = zu || (zt + '|' + zf);
        if (zk && seenD[zk]) continue;
        if (zk) seenD[zk] = true;
        nd.uniche++;
      }
      nd.duplicate = nd.grezze - nd.uniche;
    }
    out.newsDedup = nd;
  } catch (e4) { out.newsDedupErrore = e4.message; }
  return out;
}
