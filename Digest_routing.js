...
function sendDigestAuto2coorti(opts) {
  ...
  try {
    ...
    // v4.28.33 — IL DIGEST NON È FATTO SOLO DI NEWS. Finora l'invio ai lettori
    // era subordinato a hasItems, cioè alla presenza di NEWS: con zero news il
    // digest non partiva, anche quando bandi, norme, podcast, video, libri e
    // segnalazioni avevano contenuto da mostrare. Da qui il "non crea il mix".
    // Ora si guarda a TUTTO il contenuto disponibile: il digest parte se c'è
    // qualcosa da dire, indipendentemente dalla sezione che lo fornisce.
    var _sezioniExtraVuote = true;
    try {
      if (typeof digestSezioniStato === 'function') {
        var _st = digestSezioniStato();
        ['bandi', 'norme', 'podcast', 'video', 'libri', 'segnalazioni'].forEach(function (k) {
          if (_st[k] && _st[k].righe > 0) _sezioniExtraVuote = false;
        });
      }
    } catch (_eSez) { Logger.log('[DIGEST] stato sezioni: ' + _eSez.message); }
    var hasContenuto = hasItems || !_sezioniExtraVuote;
    if (!hasItems && hasContenuto) {
      Logger.log('sendDigestAuto2coorti: nessuna NEWS, ma le altre sezioni hanno contenuto — il digest parte comunque.');
    }
    if (!hasContenuto) Logger.log('sendDigestAuto2coorti: nessun contenuto in nessuna sezione — invio solo digest Matrix personalizzati.');
...
/**
 * v4.28.36 — Aggregazione: 1 chiamata frontend anziché 3 (KPI + queue + quota).
 * Risparmia 2 round-trip google.script.run (~4s totali di cold-start overhead).
 */
function getDigestDashboardAll() {
  return {
    kpi: getDigestDashboardKpi(),
    queue: adminGetDigestQueue({ statusFilter: 'draft' }),
    quota: getEmailQuotaRemaining()
  };
}
