...
function _generateDigestDraftCore_(opts) {
  ...
  // v4.28.34 — FIX pool troppo piccolo: bandiUrg.news/podcast sono il top-5/top-3
  // della HOME (getHomepageDataV42, pensato per la vetrina), non un bacino per il
  // digest. I filtri a valle (community, ambito, anti-ripetizione 45gg, max-2-per-
  // fonte) applicati su soli 5 elementi svuotavano spesso la sezione, mentre i
  // bandi restavano popolati perché già usano una lista vera (getUltimiBandiV5).
  // Ora si parte da un bacino reale (foglio Items/Podcast) e si tronca a
  // maxNews/maxPodcast SOLO alla fine, dopo tutti i filtri (vedi sotto).
  var newsPool = _safeCall_(function(){ return getNewsListV42(150); }, []);
  var podPool  = _safeCall_(function(){ return getPodcastListV42(80); }, []);
  var news = newsPool.filter(_noCommunity);
  var pod  = podPool;
...
function _requestSendAuthorizationCore_(draftId) {
  ...
  var webUrl = '';
  try {
    // QA 19/08 — getService().getUrl() da trigger restituisce /dev, non /exec.
    // ADMTK_PROD_URL e' l'/exec fisso, gia' dichiarato in AdminToken_v1.js.
    webUrl = (typeof ADMTK_PROD_URL === 'string' && ADMTK_PROD_URL)
           ? ADMTK_PROD_URL
...
function adminConfirmSendWithToken(draftId, authToken) {
  ...
  // un invio riuscito: si segnala come warning, non come errore.
    // QA 19/08 — se non parte NESSUNA mail non si segna "inviato": la bozza resta
  // approvabile e l'errore diventa visibile invece di sparire in silenzio.
  if (!res || !res.count) {
    draft.stato = 'in_attesa_approvazione';
    try { PropertiesService.getScriptProperties().setProperty(OC_DRAFT_PROP_PFX_ + draftId, JSON.stringify(draft)); } catch(_){}
    return { ok:false, error:'nessuna mail inviata',
  ...
  draft.sentTo = res.count || 0;
    // QA 19/08 — persistere SUBITO: se un passo successivo (social, registri)
  // va in timeout, la newsletter risulta comunque inviata e la bozza non
  // resta congelata su invio_in_corso.
  try { PropertiesService.getScriptProperties().setProperty(OC_DRAFT_PROP_PFX_ + draftId, JSON.stringify(draft)); } catch(_){}
