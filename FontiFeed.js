...
function updateFeedSourceStats(tipo, fonte, esito, nRecord, errore) {
  ...
  for (var r=1; r<v.length; r++) {
    ...
    if (!hit) continue;
    var rr = r + 1;
    // v4.28.36: batch write — 1 setValues anziché 6 setValue (risparmia ~270 API call/run)
    var row = v[r].slice(); // copia riga corrente
    row[iScan] = new Date();
    row[iEsito] = esito;
    row[iTot] = Number(v[r][iTot] || 0) + nRecord;
    row[iUlt] = nRecord;
    row[iFail] = esito === 'OK' ? 0 : Number(v[r][iFail] || 0) + 1;
    if (errore) row[iErr] = String(errore).substring(0, 120);
    sh.getRange(rr, 1, 1, row.length).setValues([row]);
    return;
  }
}
