...
// Keyword NORMATIVE forti: un item entra in Norme solo se il TITOLO contiene
// un riferimento normativo esplicito. Il sommario AI tende a inserire frasi
// boilerplate ("normativa vigente", "quadro normativo") che producono falsi positivi.
// v4.28.51 — FIX falsi positivi:
//   - rimosso `normativ` generico (matchava "aggiornamento normativo" in qualsiasi news)
//   - `regolament[oi]` ora richiede qualificazione (UE/europeo/museale/n./codice)
//   - aggiunta `normativa\s+(?:beni|cultura|museal|patrimoni)` come alternativa mirata
var _NORME_RE = /\b(decret[oi]|d\.?\s?lgs|d\.?\s?l\.|d\.?\s?m\.|d\.?p\.?c\.?m|d\.?p\.?r|circolar[ei]|regolamento\s+(?:ue|europeo|museale|n\.?\s*\d|del\s+(?:codice|ministero))|direttiva\s+(?:ue|europea)|legge\s+n|l\.\s?\d|art\.?\s?bonus|art\s+bonus|codice\s+dei\s+beni\s+cultural|d\.?lgs\.?\s*42\/2004|gdpr|privacy\s+by\s+design|wcag|peba\b|eaa\b|european\s+accessibility\s+act|ai\s+act|codice\s+appalti|d\.?lgs\.?\s*36\/2023|terzo\s+settore|d\.?lgs\.?\s*117|linee\s+guida\s+(?:mic|ministeri\w*|agid)|standard\s+icom|luq\b|livelli\s+uniformi|normativa\s+(?:dei\s+)?(?:beni|cultura|museal|patrimoni|bibliotec))/i;

// Contesto CULTURA obbligatorio (evita norme generiche non pertinenti).
// v4.28.51 — rimossi termini troppo generici: `arte`, `creativ`, `editori`, `teatr`,
// `cinema`, `spettacol` sono presenti nella quasi totalità delle news culturali
// e rendevano il filtro irrilevante.
var _NORME_CULTURA_RE = /\b(musei?|museal|patrimoni|beni\s+cultural|bibliotec|archiv|soprintendenz|mic\b|ministero\s+della\s+cultura|accessibilit|restaur|paesagg|espositiv|collezion|unesco|icom)/i;

// ANTI-RUMORE: newsletter generiche, alert advocacy USA, notizie estere non IT/UE.
// v4.28.51 — aggiunto anti-rumore italiano (comunicati stampa, inaugurazioni, mostre)
var _NORME_ESCLUDI_RE = /\b(notiziario|newsletter|advocacy\s+alert|take\s+action|congress|appropriations|\bomb\b|federal\s+grant|uniform\s+guidance|invite\s+congress|capitol\s+hill|\bh\.r\.|\bs\.\s?\d+\s+bill|comunicato\s+stampa|inaugurazion|apertura\s+mostra|apre\s+al\s+pubblico|in\s+mostra\s+a)\b/i;
 ...
 * v4.28.51 — FIX falsi positivi: la regex normativa (_NORME_RE) si applica
 * SOLO al titolo. Il sommario AI contiene spesso frasi boilerplate
 * ("normativa vigente", "quadro normativo") che producevano falsi positivi.
 * Il contesto cultura (_NORME_CULTURA_RE) si applica a titolo+sommario.
 */
function _ncIsNorma_(titolo, sommario) {
  var t = String(titolo || '').trim();
  var s = String(sommario || '').trim();
  if (!t && !s) return false;
  var tutto = t + ' ' + s;
  if (_NORME_ESCLUDI_RE.test(tutto)) return false;
  // La keyword normativa DEVE apparire nel titolo (non nel sommario AI)
  if (!_NORME_RE.test(t)) return false;
  // Il contesto cultura può apparire ovunque
  return _NORME_CULTURA_RE.test(tutto);
}
