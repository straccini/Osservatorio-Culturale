...
function frBackfillTier(opts) {
  ...
  try {
    ...
    // v4.28.44 — batch write: accumuliamo tier, 1 setValues anziché N setValue (167→1 API call)
    var tierCol = vals.slice(1).map(function(row) { return [String(row[iPri] || '')]; });
    ...
    if (!opts.dryRun && rep.assegnati > 0) sh.getRange(2, iPri + 1, tierCol.length, 1).setValues(tierCol);
