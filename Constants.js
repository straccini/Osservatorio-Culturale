...
// QA 2026-08-14 — riallineamento: OC_VERSION era ferma a v4.18.63 (16/05) mentre la
// codebase era gia' a v5.1.9 (28/05, ultimo commit 7d6d271).
//
// ⚠️ ATTENZIONE — QUESTO REPOSITORY E' INDIETRO RISPETTO ALLA PRODUZIONE.
// Deployment GAS attivo: @979-@980 (comunicato da Silvano il 14/08/2026).
// La produzione serve v4.23.0 nella sidebar: una versione che NON esiste in questo
// repository ne' in alcun punto della storia git. Il frontend live invoca 70 endpoint
// che qui non sono definiti (segnalazioni, editoriale, trend, profilo utente, login
// pubblico via email, editoria, lavoro, backup) e 5 pagine assenti dal repo.
// Le modifiche sono state fatte nell'editor web GAS e mai riportate in git.
//
// NON ESEGUIRE `clasp push` DA QUESTO REPOSITORY: cancellerebbe quel lavoro.
// Prima `clasp pull`, poi merge, poi push. Vedi QA_2026-08-14.md.
var OC_VERSION = 'v5.1.9';
var OC_VERSION_DATE = '2026-05-28';
var OC_DEPLOYMENT_GAS = '@980';   // attivo al 14/08/2026 (precedente @979)
var OC_VERSION_NOTES = 'v5.0-v5.1 (24-28/05/2026), stato locale non deployato. DESIGN v5.0 Curatoriale Sans: br-acts ridisegnati, allineamento dx mobile, micro-interazioni. FONTI: parser TED RSS + Italia Domani PNRR, OpenCoesione API + CKAN regionale, deprecazione fonti irrecuperabili, trigger fasRunCompleto giornaliero, seed fonti specializzate v2 (SeedFontiSpecializzateV2.js). AGENTE REGIONALE (AgentRegionale.js): monitor BUR + Open Data 20 regioni, rotazione 5 giorni, auto-healing, agrVerificaUrl. GAL MONITOR (GalMonitor.js): scansione bandi GAL - NON attivo, richiede galSetupTrigger() manuale. IDENTITY MAP (Identity_v1.js): filtro contenuti per ambiti/regione dell utente. BANDI: unificazione via BandiUnify.js (migrazione manuale). DIAGNOSTICA FONTI: getFontiDiagnostics() + pagina admin. DRY BACKEND: SheetHelpers.js, DigestService.js estratto da Codice.js, _cardActionsHtml_ factory, _OC_DATA + _loadList unificati. GDPR: double opt-in newsletter, digest solo a iscritti confermati, CTA header/footer + cookie dismiss 30gg. ACCESSO: auto-registrazione lettore, role guards backend, menu ospite con gate CTA, sidebar riordinata, Chi siamo pubblico. SOCIAL WALL: 10 nuove fonti cultura. Da eseguire manualmente post-deploy: setupLibriSeed, runFullMigration_Fonti, roc_setupMuseiDB, galSetupTrigger, migraBandiRadarToV5. --- STORICO v4.18.63 (11/05): Sprint 2-5 anticipati. SPRINT 2: schema unificato fonti 14 colonne (Fonti_v1.js), migrazione SocialFonti->FontiNews + split video (FontiMigration_v1.js), tab unica Tutte le Fonti in admin con filtri Tipo/Stato/Attiva + ricerca + counters dashboard. Toggle accessibilita topbar Aa+ piu visibile. SPRINT 3: Matrix radar chart Chart.js nel report con benchmark dinamico (Matrix_benchmark_v1.js - mediana/percentili compilatori). Card esplicativa Matrix in home con 3 pittogrammi + CTA dominante. SPRINT 4: CRM_v1.js con lead scoring automatico secondo tabella punti (compilazione+10 / opt-in followup+30 / click servizio+5 / digest open+1...). Notifica Telegram lead hot >=30pt. Privacy_v1.js con UTM tracking endpoint, right-to-be-forgotten /forget, pagina /trasparenza con dati aggregati anonimi. SPRINT 5: ROC_v1.js Radar Opportunita Cultura - 5 moduli outbound bando-driven (triage 4 filtri AND, notify Telegram, match musei, email batch personalizzate, approvazione manuale Silvano). Database MuseiDB_v1.tsv con 260 musei italiani curati. Cap pre-progettazioni 5-8/mese. Tono email noi Duemilamusei + firma Silvano. Da eseguire manualmente post-deploy: setupLibriSeed, runFullMigration_Fonti, roc_setupMuseiDB + import CSV MuseiDB_v1.tsv. Backend mancanti Sprint 1 risolti in Backend_v415.js (saveLibro, setupLibriSeed, saveNorma, invitaUtenteSendEmail, exportArchivio, emptyTrash).';
