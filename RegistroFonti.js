...
function rfMigra(opts) {
  ...
  if (shP && shP.getLastRow() > 1) {
    ...
    for (var r3 = 1; r3 < vp.length; r3++) {
      ...
      // v4.28.39 — FIX: era `fsTipoDaUrl(u, ...)` con `u` non definita → crash
      var tipoC = (typeof fsTipoDaUrl === 'function')
        ? fsTipoDaUrl(rp[pUrl], (pTipo >= 0 ? rp[pTipo] : ''))
  ...
  // 6. v4.28.39 — Fonti hardcoded (news + podcast + video) mai migrate in nessun foglio.
  // NOTA: le const di Scannerbandi.js e FontiSezioni.js sono file-scoped in V8,
  // quindi li leggiamo con accessor function globali (var) definite qui sotto.
  var _hcSeed = [];
  _rfSeedHardcoded_().forEach(function(s) { _hcSeed.push(s); });
  _hcSeed.forEach(function(s, i) {
    if (!s.url) return;
    proponi({
      id: 'RFHC_' + i, categoria: s.cat, nome: s.nome, urlFeed: String(s.url).trim(),
      formato: s.cat === 'video' ? 'youtube' : 'rss', stato: 'attiva',
      tematica: s.tematica, priorita: 1,
      origine: 'migrazione:hardcoded:' + s.src,
      note: 'Migrata da array hardcoded ' + s.src
    }, 'Hardcoded(' + s.src + ')');
  });
...
// ---------------------------------------------------------------------------
// v4.28.40 — Riallinea stato attiva/sospesa confrontando coi fogli sorgente
// ---------------------------------------------------------------------------

function rfRiallineaStato(opts) {
  opts = opts || {};
  var dryRun = opts.dryRun === true;
  var ss = getMainSS();
  var sh = ss.getSheetByName(RF_SHEET);
  if (!sh || sh.getLastRow() < 2) return { ok: false, errore: 'RegistroFonti vuoto' };

  // Costruisci set URL attive dai fogli sorgente
  var attiveUrl = {};

  // FontiFeed
  var shF = ss.getSheetByName('FontiFeed');
  if (shF && shF.getLastRow() > 1) {
    var vf = shF.getDataRange().getValues(); var hf = vf[0];
    var fUrl = hf.indexOf('URL_Feed'), fAtt = hf.indexOf('Attiva');
    for (var r = 1; r < vf.length; r++) {
      var att = vf[r][fAtt];
      if (att === true || att === 'TRUE' || att === 'true' || att === 'Sì' || att === 'Si' || att === 'si' || att === 1 || att === '1') {
        attiveUrl[_rfNorm_(vf[r][fUrl])] = true;
      }
    }
  }
  ...
  var shS = ss.getSheetByName('SocialFonti');
  if (shS && shS.getLastRow() > 1) {
    var vs = shS.getDataRange().getValues(); var hs = vs[0];
    var sUrl = hs.indexOf('URL'), sAtt = hs.indexOf('Attiva');
    for (var r2 = 1; r2 < vs.length; r2++) {
      var att2 = vs[r2][sAtt];
      if (att2 !== false && att2 !== 'FALSE' && att2 !== 'false' && att2 !== 0) {
        attiveUrl[_rfNorm_(vs[r2][sUrl])] = true;
      }
    }
  }
  ...
  var shP = ss.getSheetByName('FontiPodcast');
  if (shP && shP.getLastRow() > 1) {
    var vp = shP.getDataRange().getValues(); var hp = vp[0];
    var pUrl = (hp.indexOf('URL_RSS') >= 0 ? hp.indexOf('URL_RSS') : hp.indexOf('URL'));
    var pAtt = hp.indexOf('Attiva');
    for (var r3 = 1; r3 < vp.length; r3++) {
      var att3 = vp[r3][pAtt];
      if (att3 === true || att3 === 'TRUE' || att3 === 'true' || att3 === 'Sì' || att3 !== false) {
        attiveUrl[_rfNorm_(vp[r3][pUrl])] = true;
      }
    }
  }
  ...
  var shL = ss.getSheetByName('Fonti');
  if (shL && shL.getLastRow() > 1) {
    var vl = shL.getDataRange().getValues(); var hl = vl[0];
    var lRss = hl.indexOf('RSSURL'), lUrl = hl.indexOf('URL'), lAtt = hl.indexOf('Attiva');
    for (var r4 = 1; r4 < vl.length; r4++) {
      if (vl[r4][lAtt] === true || String(vl[r4][lAtt]).toLowerCase() === 'true') {
        var feed = String(vl[r4][lRss >= 0 ? lRss : lUrl] || '').trim();
        if (feed) attiveUrl[_rfNorm_(feed)] = true;
      }
    }
  }

  // Hardcoded — sempre attive
  var hcFonti = _rfSeedHardcoded_();
  hcFonti.forEach(function(f) { if (f.url) attiveUrl[_rfNorm_(f.url)] = true; });

  // API — sempre attive
  RF_FONTI_API.forEach(function(a) { attiveUrl[_rfNorm_(a.url)] = true; });

  // Ora aggiorna il RegistroFonti
  var v = sh.getDataRange().getValues();
  var h = v[0].map(function(x) { return String(x || '').trim(); });
  var iUrl = h.indexOf('URL_Feed'), iStato = h.indexOf('Stato');
  var attivate = 0, totChecked = 0;

  for (var rr = 1; rr < v.length; rr++) {
    var k = _rfNorm_(v[rr][iUrl]);
    totChecked++;
    if (attiveUrl[k] && String(v[rr][iStato]).toLowerCase() !== 'attiva') {
      if (!dryRun) sh.getRange(rr + 1, iStato + 1).setValue('attiva');
      attivate++;
    }
  }

  return { ok: true, dryRun: dryRun, totChecked: totChecked, attivate: attivate,
           fontiAttiveNeiSorgenti: Object.keys(attiveUrl).length };
}

// ---------------------------------------------------------------------------
// v4.28.41 — Fix campi operativi post-migrazione
// La migrazione ha copiato dati sporchi (log diagnosi in Tematica, numeri in Ambito).
// Questa funzione riallinea i campi confrontando coi fogli sorgente originali.
// ---------------------------------------------------------------------------

function rfFixCampiOperativi(opts) {
  opts = opts || {};
  var dryRun = opts.dryRun === true;
  var ss = getMainSS();
  var sh = ss.getSheetByName(RF_SHEET);
  ...
  var v = sh.getDataRange().getValues();
  var h = v[0].map(function(x) { return String(x || '').trim(); });
  var iUrl = h.indexOf('URL_Feed'), iTema = h.indexOf('Tematica'),
      iAmb = h.indexOf('Ambiti'), iPri = h.indexOf('Priorita'), iOrig = h.indexOf('Origine');

  // Costruisci indice sorgente da FontiFeed (campi ORIGINALI puliti)
  var ffIndex = {};
  var shF = ss.getSheetByName('FontiFeed');
  if (shF && shF.getLastRow() > 1) {
    var vf = shF.getDataRange().getValues(); var hf = vf[0];
    var fUrl = hf.indexOf('URL_Feed'), fAmb = hf.indexOf('Ambito'),
        fAmbL = hf.indexOf('AmbitoLabel'), fPri = hf.indexOf('Priorita'),
        fCat = hf.indexOf('Categoria'), fNote = hf.indexOf('Note');
    for (var r = 1; r < vf.length; r++) {
      var k = _rfNorm_(vf[r][fUrl]);
      if (!k) continue;
      var catVal = fCat >= 0 ? String(vf[r][fCat] || '') : '';
      // Se la categoria contiene log di diagnosi, non è una tematica valida
      var catClean = (catVal.indexOf('[diagnosi') >= 0 || catVal.indexOf('FEED OK') >= 0) ? '' : catVal;
      ffIndex[k] = {
        ambito: fAmbL >= 0 ? String(vf[r][fAmbL] || '') : '',
        ambitoNum: fAmb >= 0 ? vf[r][fAmb] : '',
        priorita: fPri >= 0 ? Number(vf[r][fPri]) || 1 : 1,
        tematica: catClean
      };
    }
  }

  // Indice dalle fonti hardcoded (hanno tematica/ambito puliti)
  var hcIndex = {};
  _rfSeedHardcoded_().forEach(function(s) {
    if (s.url) hcIndex[_rfNorm_(s.url)] = { tematica: s.tematica || '', ambito: '' };
  });

  var fixes = 0, dettagli = [];
  for (var rr = 1; rr < v.length; rr++) {
    var ku = _rfNorm_(v[rr][iUrl]);
    var changed = false;
    var row = rr + 1;

    // Fix 1: tematica con log di diagnosi → pulisci
    var tema = String(v[rr][iTema] || '');
    if (tema.indexOf('[diagnosi') >= 0 || tema.indexOf('FEED OK') >= 0 || tema.indexOf('migrata da FontiPodcast') >= 0) {
      var nuovaTema = '';
      if (hcIndex[ku]) nuovaTema = hcIndex[ku].tematica;
      else if (ffIndex[ku] && ffIndex[ku].tematica) nuovaTema = ffIndex[ku].tematica;
      if (!dryRun) sh.getRange(row, iTema + 1).setValue(nuovaTema);
      changed = true;
    }

    // Fix 2: ambito è un numero → svuota (il numero era l'indice, non l'etichetta)
    var amb = String(v[rr][iAmb] || '');
    if (/^\d+$/.test(amb.trim())) {
      var nuovoAmb = '';
      if (ffIndex[ku] && ffIndex[ku].ambito) nuovoAmb = ffIndex[ku].ambito;
      if (!dryRun) sh.getRange(row, iAmb + 1).setValue(nuovoAmb);
      changed = true;
    }

    // Fix 3: priorità — riallinea con FontiFeed originale
    if (ffIndex[ku] && Number(v[rr][iPri]) !== ffIndex[ku].priorita) {
      if (!dryRun) sh.getRange(row, iPri + 1).setValue(ffIndex[ku].priorita);
      changed = true;
    }

    if (changed) {
      fixes++;
      if (dettagli.length < 10) dettagli.push(String(v[rr][h.indexOf('Nome')] || ku));
    }
  }

  return { ok: true, dryRun: dryRun, fontiCorrette: fixes, totale: v.length - 1, esempi: dettagli };
}

// ---------------------------------------------------------------------------
// v4.28.42 — Archivia fogli legacy (rinomina con prefisso * per 2 settimane)
// ---------------------------------------------------------------------------

function rfArchiviaFogliLegacy(opts) {
  opts = opts || {};
  var dryRun = opts.dryRun === true;
  var ss = getMainSS();
  var FOGLI_LEGACY = [
    'Fonti',           // news RSS legacy → ora in RegistroFonti (cat: news)
    'FontiFeed',       // intermedio unificato → ora in RegistroFonti
    'FontiPodcast',    // podcast legacy → ora in RegistroFonti (cat: podcast/video)
    'FontiNews',       // pianificato mai usato → ora in RegistroFonti
    'FontiVideo',      // pianificato mai usato → ora in RegistroFonti
    'SocialFonti'      // social wall → ora in RegistroFonti (cat: social)
  ];
  var rinominati = [], giaStar = [], nonTrovati = [];
  FOGLI_LEGACY.forEach(function(nome) {
    var sh = ss.getSheetByName(nome);
    var shStar = ss.getSheetByName('* ' + nome + ' (non utilizzare)');
    if (shStar) { giaStar.push(nome); return; }
    if (!sh) { nonTrovati.push(nome); return; }
    if (!dryRun) sh.setName('* ' + nome + ' (non utilizzare)');
    rinominati.push(nome);
  });
  return { ok: true, dryRun: dryRun, rinominati: rinominati, giaStar: giaStar, nonTrovati: nonTrovati,
           nota: 'I fogli rinominati restano visibili come archivio per 2 settimane. Poi si cancellano.' };
}
...
// ---------------------------------------------------------------------------
// v4.28.39 — Seed fonti hardcoded (Scannerbandi.js const sono file-scoped)
// Questa funzione è var→globale, così rfMigra() può leggerle.
// ---------------------------------------------------------------------------
var _rfSeedHardcoded_ = function() {
  var out = [];
  // Legge gli array da Scannerbandi.js tramite accessor globali
  var arrays = [
    { fn: '_rfGetFontiNewsIstituzionali_', cat: 'news', src: 'FONTI_NEWS_ISTITUZIONALI' },
    { fn: '_rfGetFontiArticoliArte_',      cat: 'news', src: 'FONTI_ARTICOLI_ARTE' },
    { fn: '_rfGetFontiAICultura_',         cat: 'news', src: 'FONTI_AI_CULTURA' },
    { fn: '_rfGetFontiPodcast_',           cat: 'podcast', src: 'FONTI_PODCAST' },
    { fn: '_rfGetCanaliVideo_',            cat: 'video', src: 'FS_CANALI_VIDEO' }
  ];
  arrays.forEach(function(a) {
    var getter = (typeof this[a.fn] === 'function') ? this[a.fn] : null;
    if (!getter) return;
    var fonti = getter();
    ...
    fonti.forEach(function(f) {
      if (!f.url) return;
      out.push({
        cat: a.cat, nome: f.nome, url: f.url,
        tematica: f.tematica || f.ambito || f.cat || 'cultura',
        src: a.src
      });
    });
  });
