...
function api_scanTed_(opts) {
  ...
  try {
    ...
    // v4.28.36: fetchAll batch — tutte le XML in parallelo anziché 50 fetch sequenziali con sleep
    var limit = Math.min(uniqueNotices.length, API_TED_MAX_DETAIL);

    // Fase 1: prepara metadata e URL XML per tutte le notice
    var noticesMeta = [];
    for (var j = 0; j < limit; j++) {
      ...
      noticesMeta.push({ pubNumber: pubNumber, htmlUrl: htmlUrl, xmlUrl: xmlUrl });
    }

    // Fase 2: fetchAll — scarica tutti gli XML in un'unica chiamata batch
    var xmlRequests = [];
    var xmlIndexMap = []; // mappa indice request → indice notice
    for (var m = 0; m < noticesMeta.length; m++) {
      if (noticesMeta[m].xmlUrl) {
        xmlRequests.push({ url: noticesMeta[m].xmlUrl, muteHttpExceptions: true, validateHttpsCertificates: true });
        xmlIndexMap.push(m);
      }
    }
    ...
    if (xmlRequests.length > 0) {
      try { xmlResponses = UrlFetchApp.fetchAll(xmlRequests); }
      catch(e) { Logger.log('api_scanTed_ fetchAll errore: ' + e.message); report.errori += xmlRequests.length; }
    }

    // Fase 3: parsa XML e scrivi bandi
    // Associa risposte alle notice tramite xmlIndexMap
    var xmlByNotice = {};
    for (var rx = 0; rx < xmlResponses.length; rx++) {
      xmlByNotice[xmlIndexMap[rx]] = xmlResponses[rx];
    }

    for (var j2 = 0; j2 < noticesMeta.length; j2++) {
      try {
        var meta = noticesMeta[j2];
        var titolo = '', ente = '', importo = '', scadenza = '', paese = '', descrizione = '';

        var xmlResp = xmlByNotice[j2];
        if (xmlResp && xmlResp.getResponseCode() === 200) {
          var xmlText = xmlResp.getContentText();
          var isEforms = (xmlText.indexOf('cbc:Name') !== -1 && xmlText.indexOf('ProcurementProject') !== -1);
          var parsed = isEforms ? api_parseTedXmlEforms_(xmlText) : api_parseTedXmlOld_(xmlText);
        ...
        if (api_isScaduto_(scadenza)) { report.filtrati++; continue; }

        var scritto = api_scriviRadarBandi_({
          titolo: titolo, ente: ente, livello: 'UE', regione: paese || '',
          settore: 'Cultura', importo: importo, scadenza: scadenza,
          fonteId: 'TED-API-v3', fonteNome: 'TED Europa v3',
          url: meta.htmlUrl || ('https://ted.europa.eu/udl?uri=TED:NOTICE:' + meta.pubNumber),
          sommario: descrizione, note: 'PubNum: ' + meta.pubNumber
        }, titoliGia);

        if (scritto) { report.nuovi++; } else { report.duplicati++; }
      } catch (e) {
        Logger.log('api_scanTed_ notice ' + j2 + ' errore: ' + e.message);
        report.errori++;
      }
    }
...
function apiScanEditoria(opts) {
  ...
  // 9. Rijksmuseum — DISABILITATO v4.28.47 (HTTP 410 GONE, API migrata su
  //    data.rijksmuseum.nl — serve nuova chiave gratuita). Riabilitare quando
  //    si registra la nuova API key e si aggiorna l'endpoint.
  // try {
  ...
  // }
  report.rijksmuseum = { ok: true, fonte: 'Rijksmuseum', nuovi: 0, duplicati: 0, errori: 0, dettagli: ['Disabilitato: API migrata (410), serve nuova chiave da data.rijksmuseum.nl'] };
  ...
  // v4.28.47 — retry (API instabile, 500 intermittenti)
  try {
    report.smk = api_scanSMK_(opts, titoliEditoria);
    Utilities.sleep(API_SLEEP_MS);
  } catch (e) {
    Logger.log('apiScanEditoria SMK primo tentativo fallito: ' + e.message + ' — retry');
    Utilities.sleep(3000);
    try {
      report.smk = api_scanSMK_(opts, titoliEditoria);
    } catch (e2) {
      report.smk = { ok: false, errori: 1, dettagli: ['Errore dopo retry: ' + e2.message] };
      Logger.log('apiScanEditoria SMK errore dopo retry: ' + e2.message);
    }
  }
